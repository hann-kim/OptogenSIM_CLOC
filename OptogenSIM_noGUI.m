function OptogenSIM(gridsearch_nm)
gridsearch_nm = str2double(gridsearch_nm);  % convert input parameter to double
...
H(22) = gridsearch_nm;   % gridsearch_nm is input parameter of this function
...
function gridSearch
% grid search that sets wavelength values and inputs them into OptogenSIM
% and has OptogenSIM.m save .csv output for each wavelength value called

% initialize starting wavelength value
nm = 300;

for i = 1:3     % change 3 to number of times you want to run
    nm = nm + 5;    % change 5 to desired wavelength increment

    % Call OptogenSIM with hard-coded wavelength value
    command = "OptogenSIM " + nm;
    system(command);    % Runs system command of OptogenSIM at nm
end
